...
const DeleteService = () => {
  ...
  useEffect(() => {
    const url = `https://serene-springs-79030.herokuapp.com/services?email=${user.email}`;
    fetch(url)
      .then((res) => res.json())
  ...
  const handleDeleteService = (id) => {
    ...
    if (proceed) {
      const url = `https://serene-springs-79030.herokuapp.com/services/${id}`;
      fetch(url, {
        method: "DELETE",
      })
        .then((res) => res.json())
